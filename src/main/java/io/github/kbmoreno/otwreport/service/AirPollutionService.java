...
import io.github.kbmoreno.otwreport.exception.InvalidCredentialsException;
...
public class AirPollutionService {
    ...
    public AirPollutionDTO getAirPollutionData(double lat, double lon) {
        String url = String.format("http://api.openweathermap.org/data/2.5/air_pollution?lat=%s&lon=%s&appid=%s", lat, lon, apiKey);

        ResponseEntity<JsonNode> httpResponse = restTemplate.getForEntity(url, JsonNode.class);
        JsonNode jsonBody = httpResponse.getBody();
        validator.validateAirPollutionData(jsonBody);
