...
public class CurrentWeatherService {
    ...
    public CurrentWeatherService(RestTemplateBuilder restTemplateBuilder,
    ...
    public CurrentWeatherDto getWeatherDataByGeo(double lat, double lon) {
        String url = String.format("https://api.openweathermap.org/data/2.5/weather?lat=%s&lon=%s&appid=%s", lat, lon, owmKey);
        ResponseEntity<JsonNode> httpResponse = restTemplate.getForEntity(url, JsonNode.class);

        JsonNode jsonBody = httpResponse.getBody();
        validator.validateCurrentWeatherData(jsonBody);
        return mapper.mapCurrentWeatherData(jsonBody);
    }

    public CurrentWeatherDto getWeatherDataByName(String cityName, String countryCode) {
