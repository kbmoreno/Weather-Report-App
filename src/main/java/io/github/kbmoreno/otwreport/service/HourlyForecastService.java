...
public class HourlyForecastService {
    ...
    public HourlyForecastService(RestTemplateBuilder restTemplateBuilder,
    ...
    public HourlyForecastDto getForecastDataByGeo(double lat, double lon) {
        String url = String.format("https://api.openweathermap.org/data/2.5/forecast?lat=%s&lon=%s&cnt=8&appid=%s", lat, lon, owmKey);
        ResponseEntity<JsonNode> httpResponse = restTemplate.getForEntity(url, JsonNode.class);

        JsonNode jsonBody = httpResponse.getBody();
        validator.validateHourlyForecastData(jsonBody);
        return mapper.mapHourlyForecastData(jsonBody);
    }

    public  HourlyForecastDto getForecastDataByName(String cityName, String countryCode) {
