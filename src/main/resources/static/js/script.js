...
async function fetchPollution(lat, lon) {
    try {
        const response = await fetch(`http://localhost:8080/api/data/ap?lat=${lat}&lon=${lon}`);
        if (!response.ok) throw new Error(`HTTP Error: ${response.status}`);
        ...
        ap.aqi = jsonData.main.aqi;
        ap.co = jsonData.components.co + " μg/m3";
        ap.no = jsonData.components.no + " μg/m3";
        ap.no2 = jsonData.components.no2 + " μg/m3";
        ap.o3 = jsonData.components.o3 + " μg/m3";
        ap.so2 = jsonData.components.so2 + " μg/m3";
        ap.pm2_5 = jsonData.components.pm2_5 + " μg/m3";
        ap.pm10 = jsonData.components.pm10 + " μg/m3";
        ap.nh3  = jsonData.components.nh3 + " μg/m3";
        return ap;
    } catch (error) {
